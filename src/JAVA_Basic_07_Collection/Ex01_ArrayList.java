...
import JAVA_Basic_07_Collection.kosa.Emp;
...
public class Ex01_ArrayList {
	...
	public static void main(String[] args) {
		...
		list.add(new Emp(100, "김완", "IT"));
		list.add(new Emp(200, "김이", "IT"));
		for (int i = 0; i < list.size(); i++) {
			//Object obj = list.get(i); <제너릭>이 없으면 object 반환
			//Emp emp = (Emp) obj; 다운캐스팅 해줘야됨
			//System.out.println(emp.getEmpno());
		}

		//Generic (제너릭): 타입 강제
		List<Emp> list2 = new ArrayList<>();
		list2.add(new Emp(100, "김완", "IT"));
		for (Emp emp : list2) {
			System.out.println(emp.getEmpno());
		}
	}
