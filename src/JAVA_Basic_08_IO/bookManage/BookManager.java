package JAVA_Basic_08_IO.bookManage;
...
public class BookManager {
    ...
    public void run(){
        while(true){
            //System.out.println("사이즈" + bookMap.size());
    ...
    public void loadBook() {
        ...
        try{
            ...
            bookMap = (HashMap)oos.readObject();

            Set<String> bookStr = bookMap.keySet();
            for (String s : bookStr) {
                System.out.println(bookStr.size());
                String isbn = bookMap.get(s).isbn;
                String bookName = bookMap.get(s).title;
                int price = bookMap.get(s).price;
