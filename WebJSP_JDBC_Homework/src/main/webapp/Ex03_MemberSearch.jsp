...
<%@ page import="kr.or.dto.MemberSearchDTO" %>
				...
				<table style="width: 400px;height: 100px;margin-left: auto;margin-right: auto">
							...
							<%
								if(rowcount > 0){
									...
									rs = pstmt.executeQuery();
									List<MemberSearchDTO> memberSearchDTOList = new ArrayList<>();
									while(rs.next()) {
										String id = rs.getString(1);
										String mname = rs.getString(2);
										String email = rs.getString(3);
										memberSearchDTOList.add(new MemberSearchDTO(id, mname, email));
									}
									request.setAttribute("memberSearchDTOList",memberSearchDTOList);
							%>
							<c:forEach var="member" items="${memberSearchDTOList}">
								<tr>
									<td>${member.id}</td>
									<td>${member.mname}</td>
									<td>${member.email}</td>
